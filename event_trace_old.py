...
class Token(object):
    ...
    def simulation(self): ### action = {'priority': priority}
        ...
        transition = (self._params.INDEX_AC[self._trans.label], resource._get_name())
        ro_single = self._process.get_occupations_all_role(resource._get_name())
        pr_wip = self.pr_wip_initial + self._resource_trace.count
        initial_ac_wip = self._params.AC_WIP_INITIAL[self._trans.label]
